...
namespace hui
{
    public partial class FilmsByActor : Form
    {
        ...
        private void FilmsByActor_Load(object sender, EventArgs e)
        {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("FilmsByActor", connection))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter("@StartingLetter", SqlDbType.Date));
                cmd.Parameters["@StartingLetter"].Value = textBox1.Text;
                try
                {
