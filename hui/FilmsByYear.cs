...
namespace hui
{
    public partial class FilmsByYear : Form
    {
        ...
        private void FilmsByYear_Load(object sender, EventArgs e)
        {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("FilmsByYear", connection))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter("@Год", SqlDbType.Date));
                cmd.Parameters["@Год"].Value = textBox1.Text;
                try
                {
